...
void CGN_SETUP_IF cgn_setup(cgn::CGNInitSetup &x) {
    ...
    if (fdarg != api.get_kvargs().end()){
        ...
        if (ls.erase("host_tools_keep_same_config") == 1)
            x.configs["host_release"] = x.configs["DEFAULT"] = config_guessor(ls);
        else {
            auto ls_copy = ls;
            x.configs["DEFAULT"] = config_guessor(ls);
            x.configs["host_release"] = generate_host_release(ls_copy);
        }
        ...
        x.configs["DEFAULT"] = x.configs["host_release"] = generate_host_release();
