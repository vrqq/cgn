...
// \ : Escape character.
static std::string escapeForBash(const std::string& in) {
...
std::string Tools::shell_escape(
    ...
    if (shell_type == "bash")
        return escapeForBash(in);
    if (shell_type == "cmd")
        return escapeForCmd(in);
    if (shell_type == "zsh")
        return escapeForBash(in); //TODO
    if (shell_type == "powershell")
        return escapeForPowerShell(in);
    
    throw std::runtime_error{"Unsupported shell " + shell_type};
    return "";
} // Tools::shell_escape()
...
HostInfo Tools::get_host_info()
{
    HostInfo rv;
        ...
        if (std::getenv("PSModulePath")) 
            rv.shell = "powershell";
        if (std::getenv("COMSPEC")) {
            const char* comspec = std::getenv("COMSPEC");
            if (strstr(comspec, "cmd.exe"))
                rv.shell = "cmd";
        }
    #elif __APPLE__
        rv.os = "mac";
        ...
        int mib[] = { CTL_KERN, KERN_PROC, KERN_PROC_PID, getppid() };
        if (sysctl(mib, 4, &info, &size, nullptr, 0) == 0)
            rv.shell = info.kp_proc.p_comm;
    #elif __linux__
        rv.os = "linux";
        struct utsname osInfo{};
        uname(&osInfo);
...
std::string Tools::rebase_path(
    ...
    if (p.type == CGNPath::BASE_ON_OUTPUT)
        throw std::runtime_error{"Please using rebase_path(CGNTargetMaker) instead."};
...
std::string Tools::rebase_path(
    ...
    if (p.type == CGNPath::BASE_ON_OUTPUT)
        return rebase_path(p.rpath, new_base, maker->out_prefix);
    else if (p.type == CGNPath::BASE_ON_SCRIPT)
        return rebase_path(p.rpath, new_base, maker->src_prefix);
    return rebase_path(p.rpath, new_base);
}
...
void Tools::convert_cgnpath_to_working_root_inplace(
    std::vector<CGNPath> &ls,
    CGNTargetOpt *opt
) {
    for (auto &it : ls)
        it = convert_cgnpath_to_working_root(it, opt);
}
...
std::string Tools::filename_of_path(const std::string &in)
{
    auto p = std::filesystem::path{in};
...
bool Tools::is_directory_case_sensitive(const std::string& directory)
{
    ...
    std::string dir1 = directory, dir2 = directory;
    dir1.back() = std::toupper(dir1.back());
    dir2.back() = std::tolower(dir2.back());

    bool test1 = std::filesystem::exists(dir1);
    bool test2 = std::filesystem::exists(dir2);
    if (test1 && test2)
        return false;
    else if (!test1 && !test2) //unicode or other case.
        throw std::runtime_error{"dir not exist or unsupported."};
    else
        return true;
}
